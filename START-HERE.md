...
6. Read AUTONOMY.md with me and ask two questions in plain language, one at a time. First: how much do I want you doing for me, level 0 (answer my questions only) through level 4 (run the roster, everything except trades)? Recommend level 2 to start and say most people move up after a week. Second: which model tier, best, balanced or economy? Explain what each costs me in quality and spend. Third: how experienced a fantasy player am I, new (never played, or one season without paying much attention), casual (play every year, set a lineup, do not study it), experienced (know the formats, follow the news, think in value), or expert (several leagues, probably know more than you about some of them)? Say plainly that there is no better or worse answer here and give no recommendation. Then write all three answers into league/standing-authorizations.md with the date, and only set up the crons and authorities that my level actually calls for (rules/autonomy-levels.md). My experience answer sets how much every later answer explains itself, per rules/experience-levels.md: if I said new or casual, define each term the first time it comes up and add one plain sentence about the concept behind every move you make or propose, so I learn the game while you play it; if I said expert, skip the explanations and lead with the numbers. Trades are excluded at every level.
7. Set the reminders, including a weekly trade scan the day after the scoring period turns over (rules/weekly-trade-scan.md). Football: one before each kickoff window that touches my roster this week, an hourly league watch, a Tuesday morning waiver check. Basketball or baseball: a morning lineup pass and a second pass an hour before the first game each day, the hourly watch, and a weekly pass on the day the matchup period turns over (rules/daily-sports.md). Tell me they live only in this session and that you will recreate them after any restart.
...
11. If I am on ESPN with the MCP server installed, tell me once that I can also ask ChatGPT about my league from my phone, read only, over OpenAI's Secure MCP Tunnel, and offer to set it up now or later (CHATGPT-TUNNEL.md). If I say yes: install tunnel-client for my platform and check it against the release SHA256SUMS before extracting, walk me through creating the tunnel and picking the right ChatGPT workspace, have me store the runtime API key with tools/set-runtime-key.ps1 or tools/set-runtime-key.sh (never ask me to paste a key into the chat, and never read the key file yourself), create the profile pointing at tools/chatgpt-tunnel-entry.mjs, run doctor, start it, and confirm the server reports writes disabled before I connect it in ChatGPT.
12. Commit and push everything except .env, then give me a one screen summary of how you will operate from here and what you need from me each week.
